        ...
        <q-card flat class="border-radius-16 shadow-1 sticky-card">
          ...
          >
            <q-avatar size="140px" class="shadow-3 q-mb-md profile-avatar">
              ...
              <div
                v-else
                :class="`bg-${getAvatarColor(member.id)} text-white full-width full-height flex flex-center text-h3`"
              >
            ...
            <div class="text-h5 text-weight-bold text-grey-9 member-name">
              {{ member.full_name }}
            </div>
            ...
            <div class="row justify-center q-gutter-sm q-mt-md">
              ...
              <q-chip
                v-if="member.pgd && !member.pgd.includes('Não')"
                color="deep-purple-1"
            ...
            <q-tooltip class="bg-primary"
              >Ver perfil completo no SUAP</q-tooltip
            >
          </q-card-section>
          ...
          <q-card-section class="q-pa-md">
              ...
              <div class="row items-center q-mb-sm">
                <div class="q-pl-lg">
                  <div class="text-weight-bold text-grey-9">
                    {{ member.department?.name || member.department_id }}
                  </div>
                  <div class="text-caption text-grey-7">
                    {{ member.workstation || "Campus Cidade de Goiás" }}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="info-group q-mb-md">
              ...
              <div class="row items-start q-mb-sm">
                <div class="q-pl-lg">
                  <div
                    class="text-body2 text-grey-9 text-weight-medium whitespace-pre-wrap line-height-1-4"
                  >
      ...
      <div class="col-12 col-md-8">
        <q-card flat class="border-radius-16 shadow-1 q-mb-md">
          ...
          <q-card-section class="q-pt-none">
            ...
            <div
              v-if="member.role || member.access_rule"
              class="row q-gutter-sm q-mt-md q-px-sm"
            >
              <q-badge
                v-if="member.role"
                color="blue-grey-1"
                text-color="blue-grey-9"
                class="q-pa-sm border-radius-8"
              >
                <q-icon name="security" class="q-mr-xs" />
                <span class="text-weight-medium">Função:</span>
                {{ member.role }}
              </q-badge>
              <q-badge
                v-if="member.access_rule"
                color="orange-1"
                text-color="orange-10"
                class="q-pa-sm border-radius-8"
              >
                <q-icon name="rule" class="q-mr-xs" />
                <span class="text-weight-medium">Regra:</span>
                {{ member.access_rule }}
              </q-badge>
            </div>
          </q-card-section>
          ...
          <q-list class="q-px-sm q-pb-md">
            ...
            >
              ...
              <q-item-section>
                ...
                <q-item-label class="text-weight-medium text-green-9">{{
                  member.phone
                }}</q-item-label>
              </q-item-section>
              <q-item-section side
...
const openSuapProfile = () => {
  if (member.value?.id) {
    window.open(
      `https://suap.ifg.edu.br/rh/servidor/${member.value.id}`,
      "_blank",
    );
  }
};
